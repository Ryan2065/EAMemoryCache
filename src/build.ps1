...
$null = Copy-Item -Path "$Net47PublishFolder\*" -Destination "$($PSScriptRoot)\Module\EAMemoryCache\Dependencies\net472\"  -Force -Recurse
$null = Copy-Item -Path "$Net6PublishFolder\*" -Destination "$($PSScriptRoot)\Module\EAMemoryCache\Dependencies\net6.0\"  -Force -Recurse

Import-Module "$PSScriptRoot\Module\EAMemoryCache\EAMemoryCache.psm1"
...
New-ExternalHelp -Path "$PSScriptRoot\docs" -OutputPath $PSScriptRoot\Module\EAMemoryCache\en-us -Force